...
The Kaggle smoke gate uses the authoritative runner (`scripts/run_kaggle_smoke.py` -> `scripts/gates/run_kaggle_t4x2.py`) wrapped by the synchronized competition notebooks (`notebooks/kaggle_t4x2_smoke.ipynb` / alias `notebooks/kaggle_smoke.ipynb`). Colab gates use `notebooks/colab_t4_smoke.ipynb` (Single-T4) and `notebooks/colab_a100_train.ipynb` (A100 production). All notebooks are generated by `scripts/generate_notebooks.py` (zero-drift enforced).

### 5.1 Kaggle Kernel Setup
1. **Notebook**: Open `notebooks/kaggle_t4x2_smoke.ipynb` (or alias `notebooks/kaggle_smoke.ipynb`).
2. **Datasets**:

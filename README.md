...
The Kaggle smoke gate uses the authoritative runner (`scripts/run_kaggle_smoke.py` -> `scripts/gates/run_kaggle_t4x2.py`) wrapped by the synchronized competition notebook (`notebooks/kaggle_t4x2_smoke.ipynb`). Colab A100 production uses `notebooks/colab_a100_train.ipynb`. All notebooks are generated by `scripts/generate_notebooks.py` (zero-drift enforced). Kaggle T4x2 is the sole pre-A100 hardware gate (the single-T4 notebook is retired).
